...
def get_check_points(local_dir, after_check_point=-1):
    """
    Get prediction checkpoints from a local eval directory.
    """
    check_points = []
    if os.path.exists(local_dir):
        for fname in os.listdir(local_dir):
            if "_predictions" in fname and "_predictions_clean" not in fname:
                try:
                    check_point = int(fname.split("_predictions")[0].split("_")[-1])
...
def get_gold_single_input_task_full(task_name, data_version, data_split):
    if task_name in ["freeform", "yesno"]:
        file_path = os.path.join(PROJECT_ROOT, "data", f"{data_version}_full", task_name, f"{data_split}.tsv")
    else:
        file_path = os.path.join(PROJECT_ROOT, "data", f"{data_version}_full", task_name,
                                 f"{data_split}.{task_name}.tsv")
...
def get_gold_moral_comparison_full(data_version, data_split, is_noisy=True):
    file_path = os.path.join(PROJECT_ROOT, "data", f"{data_version}_full", "moral_comparison",
                             f"{data_split}.moral_comparison.tsv")
    print(file_path)
    df_data = pd.read_csv(file_path, sep="\t")
    if is_noisy:
...
def get_gold_single_input_task_full_wild_v11(task_name, data_version, data_split, is_full="full"):
    if data_split == "validation":
        file_path = os.path.join(PROJECT_ROOT, "data", f"{data_version}_full", "wild", "dev.tsv")
    else:
        file_path = os.path.join(PROJECT_ROOT, "data", f"{data_version}_full", "wild", f"{task_name}.tsv")
    df_data = pd.read_csv(file_path, sep="\t")
...
def main_moral_acceptability(base_path, check_point, data_version, data_split,
                             get_gold_class, get_pred_class,
                             get_gold_text, get_pred_text,
    ...
    if is_include_class:
        inputs, targets, preds = evaluate_single_input_task_class_task(base_path, check_point,
                                                                       data_version, task_name,
                                                                       data_split,
                                                                       get_gold_class,
                                                                       get_pred_class)
    ...
    if is_include_text:
        inputs, targets, targets_class, preds, preds_class = \
            evaluate_moral_acceptability_text(base_path, check_point,
    ...
    if is_include_class or is_include_text:
        save_result_file(df_moral_acceptability_full, base_path, check_point, task_name, data_split)


def main_wild_v11(base_path, check_point, data_version, data_split,
                  get_gold_class, get_pred_class,
                  get_gold_text, get_pred_text, task_name):
    print("~" * 30, task_name)
    ...
    inputs, targets, targets_class, preds, preds_class = \
        evaluate_wild_v11_text(base_path, check_point,
                               data_version, data_split,
                               get_gold_text, get_pred_text, task_name)

    df_full = merge_wild_v11_text_results(df_full, inputs, targets, preds, targets_class, preds_class)
    save_result_file(df_full, base_path, check_point, task_name, data_split)


def main_moral_agreement(base_path, check_point, data_version, data_split,
                         get_gold_class, get_pred_class,
                         get_gold_text, get_pred_text,
    ...
    if is_include_class:
        ...
        df_moral_agreement_full = merge_single_input_task_class_results(df_moral_agreement_full, inputs, targets,
                                                                    preds, task_name)
    if is_include_text:
        inputs, targets, preds = evaluate_moral_agreement_text(base_path, check_point,
                                                               data_version, data_split,
                                                               get_gold_text, get_pred_text,
                                                               task_name)
    ...
    if is_include_class or is_include_text:
        save_result_file(df_moral_agreement_full, base_path, check_point, task_name, data_split)


def main_moral_comparison(base_path, check_point, data_version, data_split,
                          get_gold_class, get_pred_class):
    print("~" * 30, "moral_comparison")
    df_moral_comparison_full = get_gold_moral_comparison_full(data_version, data_split)
    inputs, targets, preds = evaluate_moral_comparison_class(base_path, check_point,
                                                             data_version, data_split,
                                                             get_gold_class,
                                                             get_pred_class)
    df_moral_comparison_full = merge_moral_comparison_class_results(df_moral_comparison_full, inputs,
                                                                    targets, preds)
    save_result_file(df_moral_comparison_full, base_path, check_point, "moral_comparison", data_split)


################# handle result files #################
def save_result_file(df_to_save, base_path, check_point, task_name, data_split):
    result_base_path = base_path.replace(f"{data_split}_eval", task_name)
    result_dir = os.path.join(result_base_path, data_split)
    os.makedirs(result_dir, exist_ok=True)
    result_path = os.path.join(result_dir, f"{task_name}_{check_point}.csv")
...
def evaluate_single_input_task_class_task(base_path, check_point, data_version, task_name,
                                          data_split, get_gold, get_pred):
    inputs, targets = get_gold(data_version, task_name, data_split)
    preds = get_pred(base_path, task_name, check_point)
...
def evaluate_moral_acceptability_text(base_path, check_point, data_version, data_split,
...
def evaluate_moral_agreement_text(base_path, check_point, data_version, data_split,
...
def evaluate_wild_v11_text(base_path, check_point, data_version, data_split,
                           get_gold, get_pred, task_name):
    inputs, targets = get_gold(data_version, task_name, data_split)
    preds = get_pred(base_path, task_name, check_point)
...
def evaluate_moral_comparison_class(base_path, check_point, data_version, data_split,
                                    get_gold, get_pred):
    inputs, targets = get_gold(data_version, data_split)
    preds = get_pred(base_path, check_point)
    if len(inputs) != len(preds):
        print(f"ERROR: inputs {len(inputs)} and preds {len(preds)} have different length")
    return inputs, targets, preds


def get_result_check_points(local_dir, after_check_point=-1):
    check_points = []
    if os.path.exists(local_dir):
        for fname in os.listdir(local_dir):
            if fname.endswith(".csv") and ("freeform" in fname or "yesno" in fname
                    or "moral_acceptability" in fname or "moral_agreement" in fname):
                try:
                    check_point = int(fname.split(".csv")[0].split("_")[-1])
                    if check_point > after_check_point:
                        check_points.append(check_point)
                except ValueError:
                    pass
...
def read_result_file(data_version, model_type, check_point, data_split, task_name, lr, bs, pt_model):
    result_path = os.path.join(PROJECT_ROOT, "results", data_version,
                               pt_model, model_type, f"lr-{lr}_bs-{bs}",
                               task_name, data_split, f"{task_name}_{check_point}.csv")
